...
export async function enrichTabsWithHistory(tabs, settings, chromeApi = globalThis.chrome) {
  ...
  let nearbyEntries = [];
  try {
    nearbyEntries = await withTimeout(
      ...
      HISTORY_TIMEOUT_MS,
      "nearby history lookup timed out"
    );
  } catch (error) {
    warnings.push(`Could not load nearby history: ${error.message || String(error)}.`);
  }
  ...
  for (const tab of tabs) {
    ...
    try {
      ...
      warnings.push(`Could not load history for "${tab.title || tab.url}": ${error.message || String(error)}.`);
      enriched.push(tab);
    }
  }
